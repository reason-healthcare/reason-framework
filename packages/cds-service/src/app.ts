...
const endpointConfigurationFromParameters = (
  ...
  const endpoints = parameters
    .filter((p) => p.name === 'artifactEndpointConfiguration')
    ?.map((p) => {
      let artifactRoute = p.part?.find(
        (p) => p.name === 'artifactRoute'
      )?.valueUri
      let endpoint =
        p.part?.find((p) => p.name === 'endpoint')?.resource ||
        createEndpoint(
          p.part?.find((p) => p.name === 'endpointUri')?.valueUri,
          'content'
        )
    ...
    async (req: FastifyRequest, res: FastifyReply): Promise<void> => {
      ...
      if (parameters != null) {
        ...
        if (
          ...
          console.warn(
            'Need to specify either content and terminology endpoints or configurable endpoints'
          )
        }

        // Use profile as SD if provided
        let structureDefinition
        structureDefinition = resourceFromParameters(parameters, 'profile')

        // If profile not provided, use Canonical
        if (structureDefinition == null) {
          let url = valueFromParameters(parameters, 'url', 'valueUri')
          let structureDefinitionRaw = resolveFromEndpoints(
            url,
            configurableEndpoints,
            contentEndpoint
          )
  ...
  app.post(
    ...
    async (req: FastifyRequest, res: FastifyReply): Promise<void> => {
      ...
      if (parameters != null) {
        ...
        let planDefinition = resourceFromParameters(parameters, 'profile') as
        ...
        if (planDefinition == null) {
          let url = valueFromParameters(parameters, 'url', 'valueUri')
          planDefinition = (await resolveFromEndpoints(
            url,
            configurableEndpoints,
            contentEndpoint
          )) as fhir4.PlanDefinition
        ...
        const getDataRequirements = (actions: fhir4.PlanDefinitionAction[]) => {
          ...
          let canonicals = actions
            .flatMap((action) =>
              action.input?.flatMap((input) => input.profile)
            )
            .filter(notEmpty)
          const filteredCanonicals = canonicals.filter((c) => c != undefined)
          if (filteredCanonicals.length) {
            allCanonicals = [
              ...new Set(allCanonicals.concat(filteredCanonicals)),
            ]
          }
          actions.forEach((action) => {
            if (action.action) {
              allCanonicals = allCanonicals.concat(
                getDataRequirements(action.action)
              )
            }
          })
        ...
        if (profiles) {
          const bundleEntries = await Promise.all(
            profiles.map(async (p) => {
              const structureDefinition = (await resolveFromEndpoints(
                p,
                configurableEndpoints,
                contentEndpoint
              )) as fhir4.StructureDefinition
              const questionnaire: fhir4.Questionnaire =
                await buildQuestionnaire({
                  structureDefinition,
                  data,
                  dataEndpoint,
                  configurableEndpoints,
