...
import {
  ...
  buildModularQuestionnaire,
  buildModularQuestionnaireArgs,
            ...
            const tmpCards =
              flattenedRequestGroup
                ?.map((action, index) => {
                  ...
                  // Add source
                  let source: CDSHooks.Source = {
                    label: planDefinition.title ?? 'Placeholder Label',
                  }
                      ...
                      .map((suggestionAction, subIndex) => {
                        ...
                        return {
                          ...
                          actions: actions.filter(
                            (a) =>
                              Object.values(a).filter((a) => a != null).length >
                              0
                          ),
                        }
                      })
  ...
  app.post(
    ...
    async (req: FastifyRequest, res: FastifyReply): Promise<void> => {
      ...
      if (parameters != null) {
        ...
        const args: buildModularQuestionnaireArgs = {
          planDefinition,
          artifactEndpointConfigurable,
          contentEndpoint,
          terminologyEndpoint,
        ...
        res.send(await buildModularQuestionnaire(args))
      }
    }
  )
