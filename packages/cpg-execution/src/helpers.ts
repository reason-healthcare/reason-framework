...
export const is = {
  ...
  QuestionnaireItemType(
    dataType: string
  ): dataType is fhir4.QuestionnaireItem['type'] {
    return (
      dataType === 'string' ||
      dataType === 'boolean' ||
      dataType === 'group' ||
      ...
      dataType.toLowerCase() === 'attachment' ||
      dataType.toLowerCase() === 'reference' ||
      dataType.toLowerCase() === 'quantity'
    )
  },
...
export let questionnaireBaseUrl =
  process.env.BASE_URL != null
    ...
    : 'http://questionnaire-processor'
if (baseUrl.endsWith('/')) {
  baseUrl = baseUrl.slice(0, -1)
}

// Use snapshot elements as fallback to differential
export const getSnapshotDefinition = (
  snapshotElements: fhir4.StructureDefinitionSnapshot['element'] | undefined,
  ...
  snapshotElements?.forEach((e) => {
    if (
      element.sliceName &&
      element.path === e.path &&
      element.sliceName === e.sliceName
    ) {
      snapshotElement = e
    } else if (
      e.path.includes('[x]') &&
      e.id?.replace(/[a-z]+\[x\]\:/g, '') === element.path
    ) {
      snapshotElement = e
...
export const getPathPrefix = (
  path: fhir4.ElementDefinition['path']
): fhir4.ElementDefinition['path'] => {
  ...
  return artifactEndpointConfigurable.sort((a, b) => {
    const aRank =
      a.artifactRoute && canonical.startsWith(a.artifactRoute)
        ...
        : 0
    const bRank =
      b.artifactRoute && canonical.startsWith(b.artifactRoute)
        ? b.artifactRoute.length
        : 0
    return bRank - aRank
  })
}
