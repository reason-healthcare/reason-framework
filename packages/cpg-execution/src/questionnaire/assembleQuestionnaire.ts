...
import { inspect, is, notEmpty, resolveFromConfigurableEndpoints } from '../helpers'
...
export const assembleQuestionnaire = async (
  ...
  const { id, meta, implicitRules, language, text, version, url, identifier, extension, item, contained } = questionnaire
  ...
  if (extension != null) {
    const extensions = extension.map(ext => {
      if (ext.url === SDC_QUESTIONNAIRE_ASSEMBLE_EXPECTATION) {
        if (ext.valueCode === 'assemble-root') {
          return undefined
        } else if (ext.valueCode?.includes('assemble')) {
          return {...ext, valueCode: ext.valueCode.replace('assemble', 'independent')}
        } else if (ext.valueCode?.includes('assembly')) {
          return {...ext, valueCode: ext.valueCode.replace('assembly', 'independent')}
        }
      }
      return ext
    }).filter(notEmpty)
  ...
  if (item != null) {
    const items: (fhir4.QuestionnaireItem | fhir4.QuestionnaireItem[])[] = await Promise.all(
      item.map(async (i) => {
      ...
      if (subQuestionnaireExtension != null && subQuestionnaireExtension.valueCanonical != null) {
        let subQuestionnaireRaw
        if (contained != null && contained.length) {
          subQuestionnaireRaw = contained?.find(resource => is.Questionnaire(resource) && resource.url === subQuestionnaireExtension.valueCanonical)
        }
        if (subQuestionnaireRaw == null && contentEndpoint != null) {
          const resolver = Resolver(contentEndpoint)
          subQuestionnaireRaw = await resolver.resolveCanonical(subQuestionnaireExtension.valueCanonical)
        }
        if (subQuestionnaireRaw == null && artifactEndpointConfigurable != null) {
        ...
        if (is.Questionnaire(subQuestionnaireRaw)) {
          ...
          if (subQuestionnaire.language !== language) {
            return {...i, text: `Error: ${subQuestionnaireExtension.valueCanonical} language ${subQuestionnaire.language} does not match root questionnaire language ${language}`}
          }
          if (subQuestionnaire.implicitRules != null ) {
            return {...i, text: `Error: unable to process sub-questionnaire with ${subQuestionnaireExtension.valueCanonical} implicit rules`}
          }
          if (subQuestionnaire.modifierExtension != null) {
            return {...i, text: `Error: unable to process sub-questionnaire with ${subQuestionnaireExtension.valueCanonical} modifier extension`}
          }
          if (subQuestionnaire.contained != null) {
            contained?.forEach(resource => {
              if (!questionnaireAssembled.contained?.some(r => resource.id === r.id)) {
          ...
          subQuestionnaire.extension?.forEach((e: fhir4.Extension) => {
            ...
            if ((e.url === VARIABLE && !questionnaireAssembled.extension?.some(ext => {
              ext.url === VARIABLE && ext.valueExpression === e.valueExpression
            })) || e.url === QUESTIONNAIRE_CONSTRAINT) {
              (questionnaireAssembled.extension ||=[]).push(e)
            } else if (e.url === SDC_QUESTIONNAIRE_ITEM_POPULATION_CONTEXT || e.url === SDC_QUESTIONNAIRE_ITEM_EXTRACTION_CONTEXT) {
              if (questionnaireAssembled.extension?.some(ext => {
                ext.url === e.url && ext.valueExpression === e.valueExpression
              })) {
                return {...i , text: `Error: extension ${e.url} already exists. Cannot process ${subQuestionnaire.url}`}
              } else {
                (questionnaireAssembled.extension ||=[]).push(e)
              }
            }
          ...
          if (subQuestionnaire.item != null) {
            ...
            return {...i, text: `Error: ${subQuestionnaire.url} does not contain items for substitution`}
          }

        } else {
          return {...i , text: `Error: ${subQuestionnaireExtension.valueCanonical} does not seem to be a FHIR Questionnaire`}
        }
      } else {
        return i
      }
