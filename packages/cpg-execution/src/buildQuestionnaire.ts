...
export const buildQuestionnaire = async (args: BuildQuestionnaireArgs) => {
  ...
  const elementIsRootOrHasParent = (
  ...
  if (featureExpression) {
    featureExpressionResource = await processFeatureExpression(
      featureExpression,
      configurableEndpoints,
      terminologyResolver,
      contentResolver,
    ...
    if (featureExpressionResource) {
      ...
      Object.keys(featureExpressionResource).forEach((k) => {
        ...
        if (
          k !== 'extension' &&
          featureExpressionResource[k] &&
          !subGroupElements?.some((e) => e.path.startsWith(elementPath))
        ) {
          elementDef = structureDefinition?.snapshot?.element.find((el) =>
            el.path.startsWith(elementPath)
          )
        }
      ...
      const featureType = featureExpressionResource.extension?.find(
        (e: any) =>
          e.url ===
      ...
      if (
        featureType === 'asserted' ||
        data?.entry?.find(
          (e) => e.resource?.id === featureExpressionResource.id
        )
      ) {
        extractContextExtension = [
          {
            url: 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-itemExtractionContext',
            valueExpression: featureExpression
          }
        ]
      }
    }
    ...
    if (!extractContextExtension) {
      extractContextExtension = [
        {
          url: 'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-itemExtractionContext',
          valueCode: structureDefinition.type
        }
      ]
    }
  }
  ...
  if (subGroupElements && rootElement) {
    ...
    extractContextExtension
      ? (questionnaire.item[0].extension = extractContextExtension)
      : undefined
