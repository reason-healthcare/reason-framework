...
export const assembleQuestionnaire = async (
  ...
  const { questionnaire, contentEndpoint } = args

  const { version, url, extension, item } = questionnaire
  ...
  if (item) {
    ...
    for (const i of item) {
      ...
      if (subQuestionnaireExtension?.valueCanonical) {
        const subQuestionnaire = await contentResolver.resolveCanonical(
        ...
        if (subQuestionnaire?.item) {
          subQuestionnaire.item.forEach((item: fhir4.QuestionnaireItem) =>
  ...
  if (extension) {
    const assembleExpectationIndex = extension.findIndex(
      (e) =>
        e.url ===
        'http://hl7.org/fhir/uv/sdc/StructureDefinition/sdc-questionnaire-assemble-expectation'
    )
    const assembleExpectationCode =
      extension[assembleExpectationIndex].valueCode
    if (assembleExpectationCode === 'assemble-root') {
      ...
      set(
        questionnaire,
        `${extension[assembleExpectationIndex]}.valueCode`,
        assembleExpectationCode.replace('assemble', 'independent')
      )
    }
  }
  ...
  extension?.push(assembledFromExt) ||
    set(questionnaire, 'extension', [assembledFromExt])
