...
When a patient is selected from the result list the system SHALL store a patient summary object containing: `id` (`Patient.id`), `resourceType: 'Patient'`, `name` (rendered full name), `dob` (`Patient.birthDate`), `gender` (`Patient.gender`), `source: 'endpoint'`, `endpointUrl` (`dataEndpoint.address`), `json` (the minimal Patient collection Bundle serialized as JSON), and `addedAt` (ISO 8601 timestamp).
...
#### Scenario: Patient summary json field contains collection bundle
- **WHEN** a patient is selected and the summary is written to the recent patients store
- **THEN** the `json` field SHALL contain a serialized FHIR `Bundle` of type `collection` with a single entry containing the selected `Patient` resource
