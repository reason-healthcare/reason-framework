...
- **THEN** a summary object SHALL be written to `cpg-review:recent-patients:endpoint:<endpointUrl>` containing: `id` (`Patient.id`), `resourceType: 'Patient'`, `name` (given + family), `dob` (`Patient.birthDate`), `gender` (`Patient.gender`), `source: 'endpoint'`, `endpointUrl` (`dataEndpoint.address`), `json` (the minimal Patient collection Bundle serialized as JSON), and `addedAt`
...
- **AND** that summary SHALL include `id` (`Bundle.id`), `resourceType: 'Bundle'`, `name`, `source: 'package'`, `resourceCount`, `resourceTypes`, `json` (the full Bundle serialized as JSON), and `addedAt`
...
- **THEN** the apply form SHALL set `dataPayload` from the stored `json` field
- **AND** the apply form SHALL set `subject` to `Patient/<patientId>` where `patientId` is extracted from the `Patient` resource found inside the stored `json` bundle

#### Scenario: Package recent selection can fall back to resolver lookup
- **WHEN** a recent package-bundle entry has no stored `json` and the uploaded resolver is available
- **THEN** the bundle payload SHALL be recovered from `resolver.resourcesByReference[Bundle/${id}]`

#### Scenario: Missing package payload is surfaced as an error
- **WHEN** a recent package-bundle entry has neither valid stored `json` nor a resolver fallback bundle
...
### Requirement: bundleReference and bundleJson fields on package summary
**Reason**: Replaced by `id` (`Bundle.id`), `resourceType: 'Bundle'`, and `json`. The full FHIR reference is reconstructable as `Bundle/${id}`; the bundle payload is now stored under the unified `json` field present on all summary types.
**Migration**: Any existing localStorage entries missing `resourceType` SHALL be silently dropped on read. Users will see an empty recents list after upgrade; entries repopulate on next patient selection.

### Requirement: patientId field on package summary
**Reason**: `patientId` was needed because `id` previously held the bundle reference string. Now `id` holds `Bundle.id`, and the patient ID is recovered from the stored `json` bundle payload when constructing the apply subject.
**Migration**: See above — stale entries are dropped on read.
