...
- Users can accept or reject recommendations with explicit control.
...
- Agent recommendation service contract: request shape (questionnaire item + patient context) and response shape (recommended answer + rationale + optional confidence).
- UI patterns for showing recommendations and rationale at questionnaire item level.
- Accept/reject interactions and state transitions per item.
- Data model updates needed to store recommendation state and user decision for the session.
...
|------------------|---------|---------|-----------|--------|---------------| ------|
| `design-llm-integration-architecture` | Define the LLM provider abstraction, prompt construction strategy, request/response schema, and error-handling contract for questionnaire recommendations | Must be resolved first; all downstream changes depend on this contract | ready | not-started | `openspec/changes/design-llm-integration-architecture` | Cover provider swap strategy, token limits, latency expectations, and PII handling in prompts |
| `design-questionnaire-recommendation-ui` | Define item-level recommendation/rationale display and accept-reject UX | Needed to establish trust and avoid confusing interaction patterns | ready | not-started | `openspec/changes/design-questionnaire-recommendation-ui` | Include edge states (no recommendation, low confidence, LLM error) |
...
| `implement-questionnaire-recommendation-engine-integration` | Integrate recommendation service responses into the questionnaire rendering pipeline | Delivers core capability users requested | needs-design | not-started | `openspec/changes/implement-questionnaire-recommendation-engine-integration` | Requires service contract from LLM implementation change |
| `implement-accept-reject-recommendation-flow` | Persist and apply per-item user accept/reject decisions in the form experience | Ensures user agency and auditable interaction intent | needs-design | not-started | `openspec/changes/implement-accept-reject-recommendation-flow` | Must not overwrite manual edits unexpectedly |
...
## Open Questions
- Which LLM provider should be targeted first (e.g., OpenAI, Anthropic, Azure OpenAI, local/self-hosted)?
...
- Is recommendation confidence required in v1 UI?
- Should reject actions capture a reason or remain binary in first release?

## Notes
The LLM integration architecture change is the critical path blocker for all implementation work. Prioritize it alongside UI design so both tracks can inform each other early. The provider abstraction layer is essential — it insulates the rest of the system from vendor lock-in and enables local/self-hosted model usage if PII constraints require it. Prioritize transparent UX and clear user control over high automation in the first release.
